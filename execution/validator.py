...
Checks performed:
    ...
    4. REQUIRE_APPROVAL mode blocks execution and triggers notify-only path.
"""
...
def pre_trade_check(
    ...
    Args:
        ...
        require_approval: When True, always raises ValidationError; the caller
            (runner.py) intercepts this to trigger notify-only mode.
    ...
    if require_approval:
        raise ValidationError(
            f"{signal.ticker}: REQUIRE_APPROVAL=true, signal logged/notified but not executed"
        )
